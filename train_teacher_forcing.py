...
def transformer(dataloader, EPOCH, frequency, path_to_save_model, path_to_save_loss, path_to_save_predictions, device):
    ...
    for epoch in range(EPOCH + 1):
        ...
        if epoch % 100 == 0:
            ...
            prediction_humidity = scaler.inverse_transform(prediction[:,:,0].detach().cpu().numpy())
            #torch.Size([35, 1, 7])
            plot_training(epoch, path_to_save_predictions, src_humidity, prediction_humidity, sensor_number, index_in,
                          index_tar)
